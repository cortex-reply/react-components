...
Just raw text content.`,
  ...
  },
  {
    ...
    format: 'richText',
    content:
      '{"root":{"type":"root","children":[{"type":"paragraph","children":[{"type":"text","text":"This is RichText content from PayloadCMS"}]}]}}',
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {sampleDocuments.map((document) => (
        <div key={document.id} className="border rounded-lg overflow-hidden">
          <DocumentPreview document={document} renderers={customRenderers} editable={false} />
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="bg-yellow-50 dark:bg-yellow-950/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
      <p className="text-yellow-800 dark:text-yellow-200 text-sm">
        Install react-markdown to see this example:{' '}
        <code>npm install react-markdown remark-gfm rehype-highlight</code>
      </p>
    </div>
  )
}
  ...
  */
  ...
  return (
    <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
      <p className="text-blue-800 dark:text-blue-200 text-sm">
        PayloadCMS RichText renderer is already available via @payloadcms/richtext-lexical
        dependency
      </p>
    </div>
  )
