...
export default function ColleaguesView({
  ...
  const handleSaveColleague = (colleague: TypedDigitalColleague) => {
    ...
    if (editingColleague) {
      ...
      setColleagues((prev) =>
        prev.map((c) => (c.id === colleague.id ? { ...colleague, type: 'digital' } : c)),
      )
      onColleagueEdit?.({ ...colleague, type: 'digital' })
    } else {
      // Add new colleague
      setColleagues((prev) => [...prev, { ...colleague, type: 'digital' }])
      onColleagueAdd?.({ ...colleague, type: 'digital' })
    }
    setCurrentView('main')
    setEditingColleague(null)
  }
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {/* {colleagues.filter((c) => c.status === 'active').length} active */}0 active
                  </p>
                </CardContent>
              </Card>
              <Card>
