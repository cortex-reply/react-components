          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <Avatar className={cn('h-12 w-12', compact && 'h-8 w-8')}>
                <AvatarImage src="/placeholder.svg" alt={colleague.name || 'name'} />
                <AvatarFallback>
                  {colleague.name
                    ?.split(' ')
                    .map((n) => n[0])
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h3 className={cn('font-semibold text-sm truncate', compact && 'text-xs')}>
                  {colleague.name}
                </h3>
          ...
          {showActions && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem
                  onClick={() => onDelete?.(colleague.id.toString())}
          ...
          {/* {colleague.type === 'human' && colleague.skills && (
            <div className="mt-3">
              <div className="flex flex-wrap gap-1">
                {colleague.skills
                  .slice(0, isExpanded ? colleague.skills.length : 3)
                  .map((skill) => (
            ...
            <div className="mt-3">
              <div className="flex flex-wrap gap-1">
                {colleague.capabilities
                  ?.slice(0, isExpanded ? colleague.capabilities.length : 2)
                  .map((capability) => (
                    <Badge
                      key={
                ...
                {(colleague.capabilities?.length || 0) > 2 && (
                  ...
                  >
                    {isExpanded
                      ? 'Show less'
                      : `+${(colleague.capabilities?.length || 0) - 2} more`}
                  </Button>
                )}
              </div>
            </div>
