...
export function ProjectCard({ project, onOpen, onShare, className }: ProjectCardProps) {
  return (
    <motion.div whileHover={{ scale: 1.02, y: -5 }} whileTap={{ scale: 0.98 }} className={className}>
      <Card className="overflow-hidden rounded-3xl border hover:border-primary/50 transition-all duration-300">
        ...
        <CardFooter className="flex gap-2">
          <Button variant="secondary" className="flex-1 rounded-2xl" onClick={() => onOpen?.(project)}>
            Open Project
          </Button>
          <Button variant="outline" size="icon" className="rounded-2xl" onClick={() => onShare?.(project)}>
