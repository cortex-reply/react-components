...
export const PlanningView: React.FC<PlanningViewProps> = ({
  ...
  const getSprintById = (sprintId: string): Sprint | undefined => {
    return sprints.find((sprint) => sprint.id === Number(sprintId))
  }
  ...
  const getTasksByEpic = (taskList: Task[]) => {
    ...
    taskList.forEach((task) => {
      if (task.epic) {
        if (!tasksByEpic[extractId(task.epic)]) {
          tasksByEpic[extractId(task.epic)] = []
        }
        tasksByEpic[extractId(task.epic)].push(task)
      } else {
  ...
  const CompactTaskCard: React.FC<{ task: Task; showSprint?: boolean }> = ({
    ...
    const epic = task.epic ? getEpicById(extractId(task.epic).toString()) : null
    const sprint = task.sprint ? getSprintById(extractId(task.sprint).toString()) : null
      ...
      >
        <div className="space-y-2 pointer-events-none">
          <div className="flex items-start justify-between gap-2">
            <h4 className="font-medium text-sm text-foreground line-clamp-1">{task.name}</h4>
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="text-muted-foreground">
              {getDisplayName((task.assignee as any)?.value?.name)}
            </span>
          </div>
            ...
            {visibleSprints.map((sprint) => {
              ...
              return (
                <Card
                  ...
                  onDragOver={handleDragOver}
                  onDragEnter={() => handleDragEnter(sprint.id.toString())}
                  onDragLeave={handleDragLeave}
                  onDrop={(e) => handleDrop(e, sprint.id.toString())}
                  style={{
                    height:
                      heroHeight > 0
